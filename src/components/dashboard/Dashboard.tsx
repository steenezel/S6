...
import { CalendarDays, ShoppingBasket, BarChart3, TrainFront, LogOut, Sparkles } from 'lucide-react'
...
export const Dashboard = ({ familyName = 'S6.Hub', userName = 'Pieter Jan' }: DashboardProps) => {
  ...
  return (
    <div className="min-h-screen bg-[#06080F] text-slate-50 flex font-sans antialiased overflow-x-hidden relative">
      ...
      {/* Sidebar met verbeterde glassmorphism */}
      <aside className="hidden lg:flex w-72 flex-col border-r border-slate-800/40 bg-[#0B101D]/40 backdrop-blur-2xl z-30">
        <div className="px-8 py-8 flex items-center gap-4">
          <div className="h-11 w-11 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center shadow-lg shadow-emerald-500/5">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          <SidebarItem icon={CalendarDays} label="Gezinsagenda" active />
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-10 py-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex flex-col items-end mr-2">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Systeem</span>
                <span className="text-xs font-medium text-emerald-400">Gezond & Live</span>
              </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <div className="xl:col-span-8 space-y-8">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                >
                  <Card className="h-full bg-[#0B101D]/60 border-slate-700/50 backdrop-blur-xl rounded-[2.5rem] p-8 shadow-2xl hover:border-violet-500/30 transition-all group">
                    <div className="flex items-center gap-4 mb-8">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-400">Wat is de vibe vandaag?</p>
                      </div>
                    </div>
                    <div className="flex items-center justify-center h-40 rounded-3xl border-2 border-dashed border-slate-800 bg-slate-900/30">
                      <p className="text-xs font-medium text-slate-500 uppercase tracking-widest">Module in ontwikkeling</p>
                    </div>
                  </Card>
                </motion.div>
              </div>
            ...
            <div className="xl:col-span-4 space-y-8">
              ...
              {/* Extra info card voor de gezinsleden die op kot zitten */}
              <Card className="bg-emerald-500/5 border-emerald-500/20 backdrop-blur-xl rounded-[2.5rem] p-8 border-dashed">
                <h3 className="text-emerald-400 font-bold text-sm uppercase tracking-widest mb-2">Weekend Status</h3>
                <p className="text-slate-300 text-sm leading-relaxed">
                  Vergeet niet je aanwezigheid voor het weekend in te vullen in de agenda! 🏠
                </p>
              </Card>
            </div>
...
const SidebarItem = ({ icon: Icon, label, active }: SidebarItemProps) => {
  return (
    <button
      type="button"
      className={[
        'w-full flex items-center gap-4 rounded-2xl px-4 py-3.5 text-left transition-all group',
        active
          ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-lg shadow-emerald-500/5'
          : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-100 border border-transparent',
      ].join(' ')}
    >
      <Icon className={['h-5 w-5 transition-transform group-hover:scale-110', active ? 'text-emerald-400' : 'text-slate-500'].join(' ')} />
